  ...
  return (
    <div className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 group">
      <div className="relative">
        ...
        {property.premium && (
          <div className="absolute top-3 left-3 bg-linear-to-r from-yellow-400 to-amber-500 text-white text-xs font-bold px-4 py-1 rounded-full shadow">
            PREMIUM
          </div>
        )}
      ...
      <div className="p-5">
        <h3 className="font-semibold text-lg leading-tight mb-1 line-clamp-2">{property.name}</h3>
        ...
        <div className="flex justify-between items-end mb-4">
          <div>
            ...
            <p className="text-xs text-gray-500">Rs.{Math.round(property.price / 100000)} Lakh</p>
          </div>
        ...
        <div className="flex gap-2 flex-wrap mb-5">
          {property.amenities.slice(0, 3).map((amenity) => (
            <span key={amenity} className="text-xs bg-gray-100 px-3 py-1 rounded-full">
              {amenity}
            </span>
          ))}
        </div>
